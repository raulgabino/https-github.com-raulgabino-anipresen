...
import {
  ...
  generateVisualDesignAction,
  optimizeFlowAction,
  testClaudeConnectionAction,
} from "@/app/actions/claude-actions"
...
const ApiConfigPanel = ({ onConnectionChange }: { onConnectionChange: (connected: boolean) => void }) => {
  ...
  const testConnection = () => {
    startTransition(async () => {
      try {
        const result = await testClaudeConnectionAction()
  ...
  return (
    <div className="bg-gray-800 p-4 rounded border-l-4 border-blue-500">
      ...
      <div className="flex gap-2 mb-2">
        <div className="flex-1 p-2 bg-gray-700 text-white rounded font-mono text-sm">
          API Key configured via environment (CLAUDE_API_KEY)
        </div>
        <button
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-gray-300">
          {connectionMessage || (isConnected ? "Connected" : "Not connected")}
        </span>
      </div>
    </div>
  )
...
const AIControls = ({
  ...
  const [isGeneratingDesign, startGeneratingDesign] = useTransition()
  ...
  const handleAnalyzeContent = () => {
    ...
    startAnalyzing(async () => {
      try {
        const result = await analyzeContentAction(editorText)
  ...
  const handleGenerateDesign = () => {
    ...
    setError(null)
    startGeneratingDesign(async () => {
      try {
        const design = await generateVisualDesignAction(analysisResult)
        onVisualDesign(design)
      } catch (error) {
        setError(error instanceof Error ? error.message : "Design generation failed")
  ...
  const handleOptimizeFlow = () => {
    ...
    startOptimizing(async () => {
      try {
        const flow = await optimizeFlowAction([])
        onOptimizedFlow(flow)
      } catch (error) {
        setError(error instanceof Error ? error.message : "Flow optimization failed")
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 flex flex-col items-center gap-6 p-6">
          ...
          <div className="text-white font-mono text-sm max-w-4xl text-center space-y-2">
            ...
            <p>
              <strong>AI Integration:</strong> Claude API para análisis inteligente de contenido (Server-side)
            </p>
